// Archivo de datos NutriPlan AI. Generado para carga directa.
// La variable 'foodCategories' no se incluye aquí ya que se importa desde la app.

const foodDatabase = {
    p0_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0: {
    "id": "p0_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0",
    "name": "Tostada de Centeno con Aguacate y Atún",
    "calories": 315,
    "protein": 30,
    "carbs": 27,
    "fats": 10,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682aff33e819fcc7a1a8d946.png",
    "ingredients": [
        {
            "name": "Tostada de Pan de Centeno",
            "quantity": "50",
            "unit": "g",
            "calories": 125.33,
            "protein": 4,
            "carbs": 22.53,
            "fats": 1.53
        },
        {
            "name": "Aguacate",
            "quantity": 50,
            "unit": "g",
            "calories": 80,
            "protein": 1,
            "carbs": 4.5,
            "fats": 7.5
        },
        {
            "name": "Atún enlatado al natural",
            "quantity": 100,
            "unit": "g",
            "calories": 110,
            "protein": 25,
            "carbs": 0,
            "fats": 1
        }
    ],
    "preparation": "1. Tostar el pan de centeno hasta que esté crujiente. 2. Triturar o cortar el aguacate y extenderlo sobre la tostada. 3. Escurrir el atún y desmenuzarlo, colocándolo encima del aguacate. 4. Opcional: añadir una pizca de sal, pimienta negra o unas hojitas de cilantro fresco para potenciar el sabor.",
    "originalBaseRecipeId": "des_tostada_de_centeno_con_aguacate_y_atn",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p0_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1: {
    "id": "p0_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1",
    "name": "Ensalada de Lentejas",
    "calories": 349,
    "protein": 26.6,
    "carbs": 40.8,
    "fats": 10,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682aff5a365e949c0a73d706.png",
    "ingredients": [
        {
            "name": "Lentejas cocidas",
            "quantity": "150",
            "unit": "g",
            "calories": 174,
            "protein": 13.5,
            "carbs": 30,
            "fats": 0.6
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.6,
            "carbs": 4.5,
            "fats": 0.1
        },
        {
            "name": "Queso de Burgos 0%",
            "quantity": 100,
            "unit": "g",
            "calories": 70,
            "protein": 12,
            "carbs": 4,
            "fats": 0.5
        },
        {
            "name": "Aguacate",
            "quantity": 25,
            "unit": "g",
            "calories": 40,
            "protein": 0.5,
            "carbs": 2.3,
            "fats": 3.8
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Si usas lentejas de bote, enjuagarlas bien bajo el grifo y escurrirlas. Si las cocinas, déjalas enfriar. 2. Picar el tomate y la cebolla (preferiblemente morada) en dados pequeños. Cortar el queso de Burgos 0% y el aguacate en dados. 3. En un bol grande, mezclar con cuidado las lentejas, el tomate, la cebolla, el queso de Burgos y el aguacate. 4. Aliñar con aceite de oliva virgen extra, vinagre (de manzana o Jerez) y sal al gusto. Opcional: añadir cilantro fresco picado o comino molido para un toque diferente.",
    "originalBaseRecipeId": "pp_ensalada_de_lentejas",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p0_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2: {
    "id": "p0_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2",
    "name": "Pechuga de Pavo a la Plancha",
    "calories": 203,
    "protein": 34.5,
    "carbs": 0,
    "fats": 7.3,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f003ef6c48dfc43a5bdb3.png",
    "ingredients": [
        {
            "name": "Pechuga de Pavo",
            "quantity": 150,
            "unit": "g",
            "calories": 158,
            "protein": 34.5,
            "carbs": 0,
            "fats": 2.3
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Si los filetes de pechuga de pavo son muy gruesos, se pueden abrir tipo libro o golpear ligeramente para que tengan un grosor más uniforme y se cocinen mejor. 2. Sazonar los filetes de pavo con sal, pimienta y tus especias favoritas (ej. orégano, pimentón dulce, curry en polvo, hierbas provenzales). 3. Calentar una plancha o sartén antiadherente a fuego medio-alto con una gota de aceite de oliva. 4. Cocinar la pechuga de pavo durante unos 3-4 minutos por cada lado, o hasta que esté dorada por fuera y completamente cocida por dentro (sin partes rosadas). El tiempo exacto dependerá del grosor.",
    "originalBaseRecipeId": "ac_pechuga_de_pavo_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p0_cena_pp_calabacn_con_cebolla_a_la_plancha_Calabac_n_con_C_mainOnly_idx3: {
    "id": "p0_cena_pp_calabacn_con_cebolla_a_la_plancha_Calabac_n_con_C_mainOnly_idx3",
    "name": "Calabacín con Cebolla a la Plancha",
    "calories": 101,
    "protein": 2.6,
    "carbs": 11.5,
    "fats": 5.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afc8e9b8ed35d8e7cbb0e.png",
    "ingredients": [
        {
            "name": "Calabacín",
            "quantity": 150,
            "unit": "g",
            "calories": 26,
            "protein": 1.8,
            "carbs": 4.7,
            "fats": 0.5
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.8,
            "carbs": 6.8,
            "fats": 0.1
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Lavar el calabacín y cortarlo en rodajas de aproximadamente 0.5 a 1 cm de grosor. Pelar y cortar la cebolla en juliana o rodajas. 2. Calentar una plancha o sartén grande con un chorrito de aceite de oliva a fuego medio-alto. 3. Colocar las rodajas de calabacín y la cebolla sobre la plancha caliente. Cocinar durante 3-5 minutos por cada lado, o hasta que estén tiernos y presenten marcas doradas de la plancha. 4. Sazonar con sal, pimienta y opcionalmente un poco de ajo en polvo u orégano durante la cocción o al final.",
    "originalBaseRecipeId": "pp_calabacn_con_cebolla_a_la_plancha",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p0_cena_ac_revuelto_de_huevos_con_esprragos_trigueros_Revuelto_de_Hue_side_idx4: {
    "id": "p0_cena_ac_revuelto_de_huevos_con_esprragos_trigueros_Revuelto_de_Hue_side_idx4",
    "name": "Revuelto de Huevos con Espárragos Trigueros",
    "calories": 290,
    "protein": 20.2,
    "carbs": 5.7,
    "fats": 20.2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f019a532eaa9f6977a78c.png",
    "ingredients": [
        {
            "name": "Espárragos Verdes",
            "quantity": 100,
            "unit": "g",
            "calories": 20,
            "protein": 2.2,
            "carbs": 3.9,
            "fats": 0.2
        },
        {
            "name": "Huevos",
            "quantity": 3,
            "unit": "unidad",
            "calories": 225,
            "protein": 18,
            "carbs": 1.8,
            "fats": 15
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Lavar los espárragos trigueros y cortar la parte inferior más dura del tallo. Trocear los espárragos en trozos de unos 2-3 cm. 2. Batir los huevos en un bol con una pizca de sal y pimienta. 3. Calentar un poco de aceite de oliva en una sartén a fuego medio. Añadir los espárragos troceados y saltear durante 3-5 minutos, o hasta que estén tiernos pero aún crujientes. 4. Verter los huevos batidos sobre los espárragos en la sartén. 5. Remover suavemente con una espátula, mezclando los huevos con los espárragos, hasta que los huevos estén cuajados al gusto. Servir caliente.",
    "originalBaseRecipeId": "ac_revuelto_de_huevos_con_esprragos_trigueros",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx5: {
    "id": "p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx5",
    "name": "Yogur Protéico (Sabores, Natural...)",
    "calories": 100,
    "protein": 15,
    "carbs": 6.3,
    "fats": 0.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd789b8ed38b127cbc09.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 100,
            "protein": 15,
            "carbs": 6.3,
            "fats": 0.6
        }
    ],
    "preparation": "1. Abrir el envase de yogur proteico. 2. Consumir directamente o añadir tus toppings favoritos si lo deseas (no incluidos en los cálculos nutricionales a menos que se especifiquen).",
    "originalBaseRecipeId": "ext_yogur_protico_sabores_natural",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p1_desayuno_des_tostada_de_centeno_con_jamn_ibrico_Tostada_de_cent_mainOnly_idx0: {
    "id": "p1_desayuno_des_tostada_de_centeno_con_jamn_ibrico_Tostada_de_cent_mainOnly_idx0",
    "name": "Tostada de centeno con jamón ibérico",
    "calories": 300,
    "protein": 19,
    "carbs": 25,
    "fats": 13.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/6887226c7537da60484d071f.png",
    "ingredients": [
        {
            "name": "Tostada de Pan de Centeno",
            "quantity": "50",
            "unit": "g",
            "calories": 125,
            "protein": 4,
            "carbs": 25,
            "fats": 1
        },
        {
            "name": "Jamón Ibérico",
            "quantity": 50,
            "unit": "g",
            "calories": 175,
            "protein": 15,
            "carbs": 0,
            "fats": 12.5
        }
    ],
    "preparation": "1. Tostar el pan de centeno hasta que esté crujiente. 2. Colocar las lonchas de jamón ibérico recién cortado sobre la tostada. 3. Servir inmediatamente.",
    "originalBaseRecipeId": "des_tostada_de_centeno_con_jamn_ibrico",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p1_comida_cc_wok_de_verduras_con_salmn_Wok_de_Verduras_mainOnly_idx1: {
    "id": "p1_comida_cc_wok_de_verduras_con_salmn_Wok_de_Verduras_mainOnly_idx1",
    "name": "Wok de Verduras con Salmón",
    "calories": 485,
    "protein": 33.5,
    "carbs": 19,
    "fats": 29.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afda0b91eb330dd3d49b2.png",
    "ingredients": [
        {
            "name": "Salmón",
            "quantity": "150",
            "unit": "g",
            "calories": 300,
            "protein": 30,
            "carbs": 0,
            "fats": 19.5
        },
        {
            "name": "Tomate",
            "quantity": 100,
            "unit": "g",
            "calories": 20,
            "protein": 1,
            "carbs": 4,
            "fats": 0
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 100,
            "unit": "g",
            "calories": 30,
            "protein": 1,
            "carbs": 6,
            "fats": 0
        },
        {
            "name": "Pimiento Verde",
            "quantity": 75,
            "unit": "g",
            "calories": 15,
            "protein": 0.75,
            "carbs": 2.25,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.75,
            "carbs": 6.75,
            "fats": 0
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Cortar todas las verduras en tiras finas. 2. Calentar un wok o sartén grande con un poco de aceite de oliva. 3. Añadir las verduras y saltear a fuego alto durante 5-7 minutos hasta que estén tiernas pero crujientes. 4. Mientras tanto, cocinar el salmón a la plancha o al vapor. 5. Añadir el salmón al wok con las verduras, mezclar bien y servir.",
    "originalBaseRecipeId": "cc_wok_de_verduras_con_salmn",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p1_cena_pp_gazpacho_Gazpacho_mainOnly_idx2: {
    "id": "p1_cena_pp_gazpacho_Gazpacho_mainOnly_idx2",
    "name": "Gazpacho",
    "calories": 163,
    "protein": 3.3,
    "carbs": 16.9,
    "fats": 10,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cfbad25d68cc1a4ec3688.png",
    "ingredients": [
        {
            "name": "Tomate",
            "quantity": 200,
            "unit": "g",
            "calories": 40,
            "protein": 2,
            "carbs": 8,
            "fats": 0
        },
        {
            "name": "Pepino",
            "quantity": 50,
            "unit": "g",
            "calories": 7.5,
            "protein": 0.5,
            "carbs": 1.5,
            "fats": 0
        },
        {
            "name": "Pimiento",
            "quantity": 25,
            "unit": "g",
            "calories": 7.5,
            "protein": 0.25,
            "carbs": 1.5,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": 25,
            "unit": "g",
            "calories": 10,
            "protein": 0.25,
            "carbs": 2.25,
            "fats": 0
        },
        {
            "name": "Ajo",
            "quantity": 5,
            "unit": "g",
            "calories": 7.5,
            "protein": 0.3,
            "carbs": 1.65,
            "fats": 0.03
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Trocear todas las verduras. 2. Poner todos los ingredientes en una batidora junto con aceite, vinagre y sal. 3. Triturar hasta obtener una sopa fina. 4. Enfriar en la nevera antes de servir. Puedes comprarlo preparado si lo prefieres.",
    "originalBaseRecipeId": "pp_gazpacho",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3: {
    "id": "p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3",
    "name": "Solomillo/Filete de Ternera a la Plancha",
    "calories": 345,
    "protein": 52,
    "carbs": 0,
    "fats": 15,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afe6c0ed5060d3743015a.png",
    "ingredients": [
        {
            "name": "Solomillo de Ternera",
            "quantity": "200",
            "unit": "g",
            "calories": 300,
            "protein": 52,
            "carbs": 0,
            "fats": 10
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Sacar el filete o solomillo de ternera del refrigerador unos 15-20 minutos antes de cocinar para que se temple. 2. Sazonar la carne con sal (preferiblemente en escamas después de cocinarla) y pimienta negra recién molida justo antes de ponerla en la plancha. 3. Calentar una plancha o sartén de fondo grueso a fuego alto hasta que esté bien caliente. Añadir una gota de aceite de oliva y extenderlo. 4. Cocinar el filete al gusto: Para poco hecho, 1-2 minutos por lado. Al punto, 2-3 minutos por lado. Bien hecho, 3-4 minutos o más por lado, dependiendo del grosor. Es importante no mover la carne mientras se sella la primera cara para obtener un buen dorado. 5. Dejar reposar la carne un par de minutos antes de servir para que los jugos se redistribuyan.",
    "originalBaseRecipeId": "ac_solomillofilete_de_ternera_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4: {
    "id": "p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4",
    "name": "Yogurt Protéico con Frutos Rojos y Secos",
    "calories": 168,
    "protein": 14.3,
    "carbs": 17.3,
    "fats": 3.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afea2b91eb35e413d4ab6.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 100,
            "protein": 12.5,
            "carbs": 7.5,
            "fats": 0.6
        },
        {
            "name": "Frutos Rojos",
            "quantity": 75,
            "unit": "g",
            "calories": 37.5,
            "protein": 0.75,
            "carbs": 9,
            "fats": 0.38
        },
        {
            "name": "Frutos Secos",
            "quantity": 5,
            "unit": "g",
            "calories": 30,
            "protein": 1,
            "carbs": 0.75,
            "fats": 2.5
        }
    ],
    "preparation": "1. Verter el yogur proteico en un bol. 2. Añadir los frutos rojos y los frutos secos por encima. 3. Servir inmediatamente.",
    "originalBaseRecipeId": "ext_yogurt_protico_con_frutos_rojos_y_secos",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0: {
    "id": "p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0",
    "name": "Revuelto de Huevos con Jamón",
    "calories": 330,
    "protein": 28,
    "carbs": 1.5,
    "fats": 23.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682ad5e1e819fcb589a8a30e.png",
    "ingredients": [
        {
            "name": "Huevos",
            "quantity": "3",
            "unit": "unidad",
            "calories": 210,
            "protein": 18,
            "carbs": 1.5,
            "fats": 15
        },
        {
            "name": "Jamón",
            "quantity": 50,
            "unit": "g",
            "calories": 75,
            "protein": 10,
            "carbs": 0,
            "fats": 3.5
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Batir los huevos, saltear el jamón y luego añadir los huevos para hacer el revuelto.",
    "originalBaseRecipeId": "des_revuelto_de_huevos_con_jamn",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p2_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1: {
    "id": "p2_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1",
    "name": "Patatas Cocidas con Salteado de Verduras y Hamburguesa a la plancha",
    "calories": 531,
    "protein": 37.3,
    "carbs": 57.3,
    "fats": 17.7,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cf8559b0c6709fc9afa98.png",
    "ingredients": [
        {
            "name": "Patata",
            "quantity": "250",
            "unit": "g",
            "calories": 192.5,
            "protein": 5,
            "carbs": 42.5,
            "fats": 0.25
        },
        {
            "name": "Pimiento Verde",
            "quantity": 75,
            "unit": "g",
            "calories": 15,
            "protein": 0.7,
            "carbs": 3.5,
            "fats": 0.15
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 75,
            "unit": "g",
            "calories": 23.25,
            "protein": 0.75,
            "carbs": 4.5,
            "fats": 0.23
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.8,
            "carbs": 6.75,
            "fats": 0.07
        },
        {
            "name": "Hamburguesa de Pavo-Pollo",
            "quantity": 150,
            "unit": "g",
            "calories": 225,
            "protein": 30,
            "carbs": 0,
            "fats": 12
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Cocer las patatas. Saltear los pimientos y la cebolla. Cocinar la hamburguesa a la plancha.",
    "originalBaseRecipeId": "cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx2: {
    "id": "p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx2",
    "name": "Patata con brócoli y pollo asado",
    "calories": 580,
    "protein": 70.2,
    "carbs": 43.9,
    "fats": 13,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687de454838c601605b6e271.png",
    "ingredients": [
        {
            "name": "Patata",
            "quantity": "200",
            "unit": "g",
            "calories": 154,
            "protein": 4,
            "carbs": 34,
            "fats": 0.2
        },
        {
            "name": "Brócoli",
            "quantity": 150,
            "unit": "g",
            "calories": 51,
            "protein": 4.2,
            "carbs": 9.9,
            "fats": 0.6
        },
        {
            "name": "Pechuga de Pollo",
            "quantity": 200,
            "unit": "g",
            "calories": 330,
            "protein": 62,
            "carbs": 0,
            "fats": 7.2
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Asar la pechuga de pollo. Cocer la patata y el brócoli al vapor o hervidos.",
    "originalBaseRecipeId": "cc_patata_con_brcoli_y_pollo_asado",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p2_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3: {
    "id": "p2_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3",
    "name": "Yogur Protéico (Sabores, Natural...)",
    "calories": 75,
    "protein": 12.5,
    "carbs": 5,
    "fats": 0.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd789b8ed38b127cbc09.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 75,
            "protein": 12.5,
            "carbs": 5,
            "fats": 0.6
        }
    ],
    "preparation": "Servir el yogur proteico frío.",
    "originalBaseRecipeId": "ext_yogur_protico_sabores_natural",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4: {
    "id": "p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4",
    "name": "Fruta Cítrica (Mandarina, Kiwi, Naranja...)",
    "calories": 88,
    "protein": 1.4,
    "carbs": 21.6,
    "fats": 0.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd3d0ed506305442ffe4.png",
    "ingredients": [
        {
            "name": "Kiwi",
            "quantity": "1",
            "unit": "unidad",
            "calories": 46,
            "protein": 0.8,
            "carbs": 11.25,
            "fats": 0.38
        },
        {
            "name": "Mandarina",
            "quantity": "1",
            "unit": "unidad",
            "calories": 42,
            "protein": 0.6,
            "carbs": 10.4,
            "fats": 0.2
        }
    ],
    "preparation": "Pelar y servir la fruta.",
    "originalBaseRecipeId": "ext_fruta_ctrica_mandarina_kiwi_naranja",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p3_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0: {
    "id": "p3_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0",
    "name": "Yogurt Protéico con Frutos Rojos y Secos",
    "calories": 313,
    "protein": 30.8,
    "carbs": 22.1,
    "fats": 13.8,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afea2b91eb35e413d4ab6.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": "250",
            "unit": "g",
            "calories": 150,
            "protein": 25,
            "carbs": 12.6,
            "fats": 1.2
        },
        {
            "name": "Frutos Rojos",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.5,
            "carbs": 4,
            "fats": 0.3
        },
        {
            "name": "Frutos Secos (Almendras)",
            "quantity": 25,
            "unit": "g",
            "calories": 143,
            "protein": 5.3,
            "carbs": 5.5,
            "fats": 12.3
        }
    ],
    "preparation": "Mezclar el yogur proteico con los frutos rojos y los frutos secos.",
    "originalBaseRecipeId": "des_yogurt_protico_con_frutos_rojos_y_secos",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p3_comida_cc_ensalada_de_pasta_con_pollo_Ensalada_de_Pas_mainOnly_idx1: {
    "id": "p3_comida_cc_ensalada_de_pasta_con_pollo_Ensalada_de_Pas_mainOnly_idx1",
    "name": "Ensalada de Pasta con Pollo",
    "calories": 617,
    "protein": 58,
    "carbs": 62.3,
    "fats": 16,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afc47b91eb349463d47dd.png",
    "ingredients": [
        {
            "name": "Pechuga de Pollo",
            "quantity": 200,
            "unit": "g",
            "calories": 220,
            "protein": 46,
            "carbs": 0,
            "fats": 4
        },
        {
            "name": "Pasta",
            "quantity": "60",
            "unit": "g",
            "calories": 210,
            "protein": 7.8,
            "carbs": 42,
            "fats": 1.2
        },
        {
            "name": "Canónigos",
            "quantity": "50",
            "unit": "g",
            "calories": 10,
            "protein": 1,
            "carbs": 1.53,
            "fats": 0.13
        },
        {
            "name": "Tomate",
            "quantity": 75,
            "unit": "g",
            "calories": 15,
            "protein": 0.8,
            "carbs": 3,
            "fats": 0.2
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 75,
            "unit": "g",
            "calories": 21,
            "protein": 0.8,
            "carbs": 4.5,
            "fats": 0.2
        },
        {
            "name": "Pimiento Verde",
            "quantity": 75,
            "unit": "g",
            "calories": 21,
            "protein": 0.8,
            "carbs": 4.5,
            "fats": 0.2
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.8,
            "carbs": 6.8,
            "fats": 0.1
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "Cocer la pasta. Cortar el pollo en tiras y cocinarlo a la plancha. Lavar y cortar los canónigos, tomate, pimientos y cebolla. Mezclar todos los ingredientes y aliñar con aceite de oliva.",
    "originalBaseRecipeId": "cc_ensalada_de_pasta_con_pollo",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p3_cena_cc_fajitas_de_pollo_y_verduras_Fajitas_de_Poll_mainOnly_idx2: {
    "id": "p3_cena_cc_fajitas_de_pollo_y_verduras_Fajitas_de_Poll_mainOnly_idx2",
    "name": "Fajitas de Pollo y Verduras",
    "calories": 445,
    "protein": 46.3,
    "carbs": 38.3,
    "fats": 12.2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afcef0ed50608b742ff33.png",
    "ingredients": [
        {
            "name": "Pechuga de Pollo",
            "quantity": 175,
            "unit": "g",
            "calories": 193,
            "protein": 40.3,
            "carbs": 0,
            "fats": 3.5
        },
        {
            "name": "Pan de Fajita",
            "quantity": 45,
            "unit": "g",
            "calories": 135,
            "protein": 3.6,
            "carbs": 22.5,
            "fats": 3.2
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 75,
            "unit": "g",
            "calories": 21,
            "protein": 0.8,
            "carbs": 4.5,
            "fats": 0.2
        },
        {
            "name": "Pimiento Verde",
            "quantity": 75,
            "unit": "g",
            "calories": 21,
            "protein": 0.8,
            "carbs": 4.5,
            "fats": 0.2
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.8,
            "carbs": 6.8,
            "fats": 0.1
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Cortar el pollo y las verduras en tiras. Cocinar el pollo y las verduras en una sartén con un poco de aceite. Calentar las tortitas de fajita. Rellenar las tortitas con el pollo y las verduras.",
    "originalBaseRecipeId": "cc_fajitas_de_pollo_y_verduras",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p3_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3: {
    "id": "p3_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3",
    "name": "Yogur Protéico (Sabores, Natural...)",
    "calories": 75,
    "protein": 12.5,
    "carbs": 6.3,
    "fats": 0.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd789b8ed38b127cbc09.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 75,
            "protein": 12.5,
            "carbs": 6.3,
            "fats": 0.6
        }
    ],
    "preparation": "Consumir directamente como snack.",
    "originalBaseRecipeId": "ext_yogur_protico_sabores_natural",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4: {
    "id": "p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4",
    "name": "Fruta Densa (Plátano, Manzana)",
    "calories": 100,
    "protein": 1.2,
    "carbs": 23,
    "fats": 0.3,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afe4bb91eb325b93d4a82.png",
    "ingredients": [
        {
            "name": "Plátano",
            "quantity": "1",
            "unit": "unidad",
            "calories": 100,
            "protein": 1.2,
            "carbs": 23,
            "fats": 0.3
        }
    ],
    "preparation": "Pelar y consumir la fruta.",
    "originalBaseRecipeId": "ext_fruta_densa_pltano_manzana",
    "isSideDish": false,
    "mealCategory": "extra"
}
};

const dailyMenus = [
  {
    name: "Libre 1",
    targetMacros: {"calories":1500,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p0_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0'],
    comida: ['p0_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1', 'p0_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2'],
    cena: ['p0_cena_pp_calabacn_con_cebolla_a_la_plancha_Calabac_n_con_C_mainOnly_idx3', 'p0_cena_ac_revuelto_de_huevos_con_esprragos_trigueros_Revuelto_de_Hue_side_idx4'],
    extra: ['p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx5'],
  },
  {
    name: "Libre 2",
    targetMacros: {"calories":1500,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p1_desayuno_des_tostada_de_centeno_con_jamn_ibrico_Tostada_de_cent_mainOnly_idx0'],
    comida: ['p1_comida_cc_wok_de_verduras_con_salmn_Wok_de_Verduras_mainOnly_idx1'],
    cena: ['p1_cena_pp_gazpacho_Gazpacho_mainOnly_idx2', 'p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3'],
    extra: ['p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4'],
  },
  {
    name: "Entreno 1",
    targetMacros: {"calories":1500,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0'],
    comida: ['p2_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1'],
    cena: ['p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx2'],
    extra: ['p2_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3', 'p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4'],
  },
  {
    name: "Entreno 2",
    targetMacros: {"calories":1500,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p3_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0'],
    comida: ['p3_comida_cc_ensalada_de_pasta_con_pollo_Ensalada_de_Pas_mainOnly_idx1'],
    cena: ['p3_cena_cc_fajitas_de_pollo_y_verduras_Fajitas_de_Poll_mainOnly_idx2'],
    extra: ['p3_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3', 'p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4'],
  }
];

// Detalles de los planes exportados:
// Plan: Libre 1, Calorías Objetivo: 1500, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150
// Plan: Libre 2, Calorías Objetivo: 1500, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150
// Plan: Entreno 1, Calorías Objetivo: 1500, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150
// Plan: Entreno 2, Calorías Objetivo: 1500, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150