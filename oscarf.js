// Archivo de datos NutriPlan AI. Generado para carga directa.
// La variable 'foodCategories' no se incluye aquí ya que se importa desde la app.

const foodDatabase = {
    p0_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0: {
    "id": "p0_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0",
    "name": "Revuelto de Huevos con Jamón",
    "calories": 403,
    "protein": 34.5,
    "carbs": 1.7,
    "fats": 29,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682ad5e1e819fcb589a8a30e.png",
    "ingredients": [
        {
            "name": "Huevos",
            "quantity": "3",
            "unit": "unidad",
            "calories": 232.5,
            "protein": 19.5,
            "carbs": 1.65,
            "fats": 16.5
        },
        {
            "name": "Jamón",
            "quantity": 50,
            "unit": "g",
            "calories": 125,
            "protein": 15,
            "carbs": 0,
            "fats": 7.5
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Preparar los ingredientes y cocinar el revuelto con un poco de aceite de oliva.",
    "originalBaseRecipeId": "des_revuelto_de_huevos_con_jamn",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p0_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1: {
    "id": "p0_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1",
    "name": "Patatas Cocidas con Salteado de Verduras y Hamburguesa a la plancha",
    "calories": 614,
    "protein": 45.8,
    "carbs": 65.7,
    "fats": 18.3,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cf8559b0c6709fc9afa98.png",
    "ingredients": [
        {
            "name": "Patata (cruda)",
            "quantity": "300",
            "unit": "g",
            "calories": 231,
            "protein": 6,
            "carbs": 51,
            "fats": 0.3
        },
        {
            "name": "Pimiento Verde",
            "quantity": 75,
            "unit": "g",
            "calories": 15,
            "protein": 0.68,
            "carbs": 3.45,
            "fats": 0.15
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 75,
            "unit": "g",
            "calories": 23.25,
            "protein": 0.75,
            "carbs": 4.5,
            "fats": 0.23
        },
        {
            "name": "Cebolla",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.83,
            "carbs": 6.75,
            "fats": 0.08
        },
        {
            "name": "Hamburguesa de Pavo-Pollo (cruda)",
            "quantity": 150,
            "unit": "g",
            "calories": 225,
            "protein": 37.5,
            "carbs": 0,
            "fats": 7.5
        },
        {
            "name": "Aceite de Oliva",
            "quantity": "10",
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "Cocer las patatas, saltear las verduras y cocinar la hamburguesa a la plancha con el aceite de oliva.",
    "originalBaseRecipeId": "cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p0_cena_cc_poke_de_arroz_con_atn_queso_feta_tomate_y_aguacate_Poke_de_Arroz_c_mainOnly_idx2: {
    "id": "p0_cena_cc_poke_de_arroz_con_atn_queso_feta_tomate_y_aguacate_Poke_de_Arroz_c_mainOnly_idx2",
    "name": "Poke de Arroz con Atún, Queso Feta, Tomate y Aguacate",
    "calories": 714,
    "protein": 49,
    "carbs": 74.4,
    "fats": 25,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cf7d29b0c67d84e9af509.png",
    "ingredients": [
        {
            "name": "Arroz (crudo)",
            "quantity": 80,
            "unit": "g",
            "calories": 288,
            "protein": 5.6,
            "carbs": 64,
            "fats": 0.8
        },
        {
            "name": "Atún (crudo)",
            "quantity": 150,
            "unit": "g",
            "calories": 195,
            "protein": 34.5,
            "carbs": 0,
            "fats": 6
        },
        {
            "name": "Queso Feta",
            "quantity": 50,
            "unit": "g",
            "calories": 132.5,
            "protein": 7,
            "carbs": 2,
            "fats": 10.5
        },
        {
            "name": "Tomate",
            "quantity": 100,
            "unit": "g",
            "calories": 18,
            "protein": 0.9,
            "carbs": 3.9,
            "fats": 0.2
        },
        {
            "name": "Aguacate",
            "quantity": 50,
            "unit": "g",
            "calories": 80,
            "protein": 1,
            "carbs": 4.5,
            "fats": 7.5
        }
    ],
    "preparation": "Cocer el arroz, cortar los ingredientes y montar el poke bowl.",
    "originalBaseRecipeId": "cc_poke_de_arroz_con_atn_queso_feta_tomate_y_aguacate",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3: {
    "id": "p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3",
    "name": "Yogur Protéico (Sabores, Natural...)",
    "calories": 75,
    "protein": 12.5,
    "carbs": 6.3,
    "fats": 0.6,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd789b8ed38b127cbc09.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 75,
            "protein": 12.5,
            "carbs": 6.25,
            "fats": 0.63
        }
    ],
    "preparation": "Servir el yogur protéico.",
    "originalBaseRecipeId": "ext_yogur_protico_sabores_natural",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p0_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4: {
    "id": "p0_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4",
    "name": "Queso Fresco Batido con Frutos Rojos y Miel",
    "calories": 118,
    "protein": 10.5,
    "carbs": 19.2,
    "fats": 0.9,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/6879302d02da47667d446b43.png",
    "ingredients": [
        {
            "name": "Queso Fresco Batido",
            "quantity": 125,
            "unit": "g",
            "calories": 62.5,
            "protein": 10,
            "carbs": 5,
            "fats": 0.63
        },
        {
            "name": "Frutos Rojos",
            "quantity": 50,
            "unit": "g",
            "calories": 25,
            "protein": 0.5,
            "carbs": 6,
            "fats": 0.25
        },
        {
            "name": "Miel Cruda",
            "quantity": "10",
            "unit": "g",
            "calories": 30.4,
            "protein": 0.03,
            "carbs": 8.2,
            "fats": 0
        }
    ],
    "preparation": "Mezclar el queso fresco batido con los frutos rojos y añadir la miel.",
    "originalBaseRecipeId": "ext_queso_fresco_batido_con_frutos_rojos_y_miel",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p1_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0: {
    "id": "p1_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0",
    "name": "Tostada de Centeno con Aguacate y Atún",
    "calories": 395,
    "protein": 28,
    "carbs": 28,
    "fats": 20,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682aff33e819fcc7a1a8d946.png",
    "ingredients": [
        {
            "name": "Tostada de Pan de Centeno",
            "quantity": 50,
            "unit": "g",
            "calories": 125,
            "protein": 4,
            "carbs": 23,
            "fats": 1
        },
        {
            "name": "Aguacate",
            "quantity": 50,
            "unit": "g",
            "calories": 80,
            "protein": 1,
            "carbs": 5,
            "fats": 8
        },
        {
            "name": "Atún (drenado)",
            "quantity": 100,
            "unit": "g",
            "calories": 100,
            "protein": 23,
            "carbs": 0,
            "fats": 1
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Tostar el pan de centeno. 2. Machacar el aguacate y untarlo sobre la tostada. 3. Escurrir el atún y colocarlo encima del aguacate. Opcionalmente, añadir sal y pimienta.",
    "originalBaseRecipeId": "des_tostada_de_centeno_con_aguacate_y_atn",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p1_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx1: {
    "id": "p1_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx1",
    "name": "Poke de aguacate, mango, tomate, soja, cebolla, arroz y pollo",
    "calories": 839,
    "protein": 71.7,
    "carbs": 82,
    "fats": 26.4,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684efa4ef6c48db833a5b1af.png",
    "ingredients": [
        {
            "name": "Aguacate",
            "quantity": 50,
            "unit": "g",
            "calories": 80,
            "protein": 1,
            "carbs": 5,
            "fats": 8
        },
        {
            "name": "Mango",
            "quantity": 50,
            "unit": "g",
            "calories": 30,
            "protein": 0,
            "carbs": 8,
            "fats": 0
        },
        {
            "name": "Tomate",
            "quantity": 75,
            "unit": "g",
            "calories": 14,
            "protein": 1,
            "carbs": 3,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 1,
            "carbs": 5,
            "fats": 0
        },
        {
            "name": "Arroz (crudo)",
            "quantity": "75",
            "unit": "g",
            "calories": 270,
            "protein": 6,
            "carbs": 60,
            "fats": 1.5
        },
        {
            "name": "Pollo (crudo)",
            "quantity": "200",
            "unit": "g",
            "calories": 330.29,
            "protein": 61.71,
            "carbs": 0,
            "fats": 6.86
        },
        {
            "name": "Salsa de soja",
            "quantity": 10,
            "unit": "ml",
            "calories": 5,
            "protein": 1,
            "carbs": 1,
            "fats": 0
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Cocer el arroz según las instrucciones del paquete. 2. Cortar el pollo en dados y cocinarlo a la plancha o cocido. 3. Picar el aguacate, mango, tomate y cebolla. 4. Mezclar todos los ingredientes en un bol, añadir el pollo cocido y la salsa de soja al gusto. Servir.",
    "originalBaseRecipeId": "cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p1_cena_pp_pimientos_salteados_con_cebolla_y_tomate_cherry_Pimientos_Salte_mainOnly_idx2: {
    "id": "p1_cena_pp_pimientos_salteados_con_cebolla_y_tomate_cherry_Pimientos_Salte_mainOnly_idx2",
    "name": "Pimientos Salteados con Cebolla y Tomate Cherry.",
    "calories": 179,
    "protein": 4.3,
    "carbs": 20,
    "fats": 10,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cd7144d6fb7451eb7bbf9.png",
    "ingredients": [
        {
            "name": "Pimiento rojo",
            "quantity": "100",
            "unit": "g",
            "calories": 31.33,
            "protein": 1.33,
            "carbs": 6,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": "100",
            "unit": "g",
            "calories": 40,
            "protein": 2,
            "carbs": 10,
            "fats": 0
        },
        {
            "name": "Tomate cherry",
            "quantity": 100,
            "unit": "g",
            "calories": 18,
            "protein": 1,
            "carbs": 4,
            "fats": 0
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Lavar las verduras y cortarlas a láminas finas a lo largo. 2. Poner un poco de aceite en una sartén y saltear las verduras.",
    "originalBaseRecipeId": "pp_pimientos_salteados_con_cebolla_y_tomate_cherry",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p1_cena_ac_merluza_a_la_plancha_Merluza_a_la_Pl_side_idx3: {
    "id": "p1_cena_ac_merluza_a_la_plancha_Merluza_a_la_Pl_side_idx3",
    "name": "Merluza a la Plancha",
    "calories": 215,
    "protein": 34,
    "carbs": 0,
    "fats": 7,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f001d4d606642b1333279.png",
    "ingredients": [
        {
            "name": "Merluza (cruda)",
            "quantity": 200,
            "unit": "g",
            "calories": 170,
            "protein": 34,
            "carbs": 0,
            "fats": 2
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Sazonar los filetes o rodajas de merluza con sal, pimienta y opcionalmente un poco de ajo en polvo o perejil picado. 2. Calentar una plancha o sartén antiadherente a fuego medio-alto con un chorrito de aceite de oliva virgen extra. 3. Cuando el aceite esté caliente, colocar la merluza en la plancha. Cocinar durante unos 3-5 minutos por cada lado, dependiendo del grosor del pescado, hasta que esté opaca, bien cocida por dentro y se desmenuce fácilmente con un tenedor. Evitar cocinarla en exceso para que no quede seca. 4. Servir inmediatamente, opcionalmente con un chorrito de zumo de limón fresco por encima.",
    "originalBaseRecipeId": "ac_merluza_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4: {
    "id": "p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4",
    "name": "Yogurt Protéico con Frutos Rojos y Secos",
    "calories": 255,
    "protein": 17.6,
    "carbs": 16.2,
    "fats": 14,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afea2b91eb35e413d4ab6.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 75,
            "protein": 13,
            "carbs": 5,
            "fats": 1
        },
        {
            "name": "Frutos Rojos",
            "quantity": "60",
            "unit": "g",
            "calories": 30,
            "protein": 0.6,
            "carbs": 7.2,
            "fats": 0
        },
        {
            "name": "Frutos Secos",
            "quantity": 25,
            "unit": "g",
            "calories": 150,
            "protein": 4,
            "carbs": 4,
            "fats": 13
        }
    ],
    "preparation": "1. En un bol o vaso, verter el yogur proteico. 2. Añadir los frutos rojos y los frutos secos por encima. 3. Mezclar suavemente o servir en capas.",
    "originalBaseRecipeId": "ext_yogurt_protico_con_frutos_rojos_y_secos",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p1_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx5: {
    "id": "p1_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx5",
    "name": "Fruta Cítrica (Mandarina, Kiwi, Naranja...)",
    "calories": 110,
    "protein": 2,
    "carbs": 24,
    "fats": 0,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd3d0ed506305442ffe4.png",
    "ingredients": [
        {
            "name": "Kiwi",
            "quantity": 1,
            "unit": "unidad",
            "calories": 40,
            "protein": 1,
            "carbs": 9,
            "fats": 0
        },
        {
            "name": "Naranja",
            "quantity": 1,
            "unit": "unidad",
            "calories": 70,
            "protein": 1,
            "carbs": 15,
            "fats": 0
        }
    ],
    "preparation": "1. Pelar y cortar la fruta cítrica elegida en gajos o rodajas. 2. Servir como snack.",
    "originalBaseRecipeId": "ext_fruta_ctrica_mandarina_kiwi_naranja",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p2_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0: {
    "id": "p2_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0",
    "name": "Yogurt Protéico con Frutos Rojos y Secos",
    "calories": 330,
    "protein": 29.1,
    "carbs": 22.3,
    "fats": 15.2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afea2b91eb35e413d4ab6.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": "250",
            "unit": "g",
            "calories": 150,
            "protein": 25,
            "carbs": 12.5,
            "fats": 1.26
        },
        {
            "name": "Frutos Rojos",
            "quantity": 75,
            "unit": "g",
            "calories": 30,
            "protein": 0.38,
            "carbs": 6,
            "fats": 0.23
        },
        {
            "name": "Frutos Secos",
            "quantity": 25,
            "unit": "g",
            "calories": 150,
            "protein": 3.75,
            "carbs": 3.75,
            "fats": 13.75
        }
    ],
    "preparation": "1. En un bol, combinar el yogur protéico. 2. Añadir los frutos rojos y los frutos secos. 3. Remover suavemente y servir.",
    "originalBaseRecipeId": "des_yogurt_protico_con_frutos_rojos_y_secos",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p2_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1: {
    "id": "p2_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1",
    "name": "Ensalada de Lentejas",
    "calories": 645,
    "protein": 43.5,
    "carbs": 86.8,
    "fats": 15.2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682aff5a365e949c0a73d706.png",
    "ingredients": [
        {
            "name": "Lentejas",
            "quantity": "125",
            "unit": "g (en crudo)",
            "calories": 425,
            "protein": 30,
            "carbs": 75,
            "fats": 1.25
        },
        {
            "name": "Tomate",
            "quantity": "50",
            "unit": "g",
            "calories": 10,
            "protein": 0.5,
            "carbs": 2,
            "fats": 0.1
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.5,
            "carbs": 4.5,
            "fats": 0.05
        },
        {
            "name": "Queso de Burgos 0%",
            "quantity": 100,
            "unit": "g",
            "calories": 60,
            "protein": 12,
            "carbs": 3,
            "fats": 0
        },
        {
            "name": "Aguacate",
            "quantity": "25",
            "unit": "g",
            "calories": 40,
            "protein": 0.5,
            "carbs": 2.25,
            "fats": 3.75
        },
        {
            "name": "Aceite de Oliva",
            "quantity": "10",
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Si usas lentejas de bote, enjuagarlas bien bajo el grifo y escurrirlas. Si las cocinas, déjalas enfriar. 2. Picar el tomate y la cebolla (preferiblemente morada) en dados pequeños. Cortar el queso de Burgos 0% y el aguacate en dados. 3. En un bol grande, mezclar con cuidado las lentejas, el tomate, la cebolla, el queso de Burgos y el aguacate. 4. Aliñar con aceite de oliva virgen extra, vinagre (de manzana o Jerez) y sal al gusto. Opcional: añadir cilantro fresco picado o comino molido para un toque diferente.",
    "originalBaseRecipeId": "pp_ensalada_de_lentejas",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p2_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2: {
    "id": "p2_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2",
    "name": "Pechuga de Pavo a la Plancha",
    "calories": 238,
    "protein": 43.8,
    "carbs": 0,
    "fats": 6.8,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f003ef6c48dfc43a5bdb3.png",
    "ingredients": [
        {
            "name": "Pechuga de Pavo",
            "quantity": 175,
            "unit": "g (en crudo)",
            "calories": 192.5,
            "protein": 43.75,
            "carbs": 0,
            "fats": 1.75
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Si los filetes de pechuga de pavo son muy gruesos, se pueden abrir tipo libro o golpear ligeramente para que tengan un grosor más uniforme y se cocinen mejor. 2. Sazonar los filetes de pavo con sal, pimienta y tus especias favoritas (ej. orégano, pimentón dulce, curry en polvo, hierbas provenzales). 3. Calentar una plancha o sartén antiadherente a fuego medio-alto con una gota de aceite de oliva. 4. Cocinar la pechuga de pavo durante unos 3-4 minutos por cada lado, o hasta que esté dorada por fuera y completamente cocida por dentro (sin partes rosadas). El tiempo exacto dependerá del grosor.",
    "originalBaseRecipeId": "ac_pechuga_de_pavo_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx3: {
    "id": "p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx3",
    "name": "Patata con brócoli y pollo asado",
    "calories": 615,
    "protein": 68.5,
    "carbs": 43,
    "fats": 17.8,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687de454838c601605b6e271.png",
    "ingredients": [
        {
            "name": "Patata",
            "quantity": "200",
            "unit": "g (en crudo)",
            "calories": 160,
            "protein": 4,
            "carbs": 36,
            "fats": 0.21
        },
        {
            "name": "Brócoli",
            "quantity": "100",
            "unit": "g",
            "calories": 35,
            "protein": 2.5,
            "carbs": 7,
            "fats": 0.4
        },
        {
            "name": "Pechuga de Pollo",
            "quantity": "200",
            "unit": "g (en crudo)",
            "calories": 330,
            "protein": 62,
            "carbs": 0,
            "fats": 7.2
        },
        {
            "name": "Aceite de Oliva",
            "quantity": "10",
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Pelar las patatas y cortarlas en cubos medianos. Cocerlas en agua hirviendo con sal durante unos 10-15 minutos o hasta que estén tiernas pero firmes. Escurrir bien. 2. Lavar el brócoli y cortarlo en floretes pequeños. Cocer al vapor o hervir en agua con sal durante 5-7 minutos hasta que esté tierno pero crujiente. 3. Precalentar el horno a 200°C. Sazonar la pechuga de pollo con sal, pimienta y hierbas al gusto. Asar en el horno durante 20-30 minutos, o hasta que esté cocida por dentro y dorada por fuera. Se puede rociar con un poco de aceite de oliva antes de asar. 4. Una vez listos todos los componentes, mezclar suavemente la patata, el brócoli y el pollo. Se puede añadir un chorrito de aceite de oliva y especias adicionales si se desea.",
    "originalBaseRecipeId": "cc_patata_con_brcoli_y_pollo_asado",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4: {
    "id": "p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4",
    "name": "Fruta Cítrica (Mandarina, Kiwi, Naranja...)",
    "calories": 45,
    "protein": 0.8,
    "carbs": 10.5,
    "fats": 0.4,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd3d0ed506305442ffe4.png",
    "ingredients": [
        {
            "name": "Kiwi",
            "quantity": 75,
            "unit": "g",
            "calories": 45,
            "protein": 0.75,
            "carbs": 10.5,
            "fats": 0.38
        }
    ],
    "preparation": "Lavar y pelar la fruta. Consumir directamente.",
    "originalBaseRecipeId": "ext_fruta_ctrica_mandarina_kiwi_naranja",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p3_comida_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx0: {
    "id": "p3_comida_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx0",
    "name": "Ensala de Patata, Atún y Huevo",
    "calories": 829,
    "protein": 68.4,
    "carbs": 76.4,
    "fats": 28.4,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afc08b91eb31afe3d4796.png",
    "ingredients": [
        {
            "name": "Patata",
            "quantity": "300",
            "unit": "g",
            "calories": 232,
            "protein": 6,
            "carbs": 51.2,
            "fats": 0.4
        },
        {
            "name": "Pimiento Verde",
            "quantity": 50,
            "unit": "g",
            "calories": 10,
            "protein": 0.5,
            "carbs": 2.3,
            "fats": 0.1
        },
        {
            "name": "Pimiento Rojo",
            "quantity": 50,
            "unit": "g",
            "calories": 16,
            "protein": 0.5,
            "carbs": 3,
            "fats": 0.2
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.6,
            "carbs": 4.7,
            "fats": 0.1
        },
        {
            "name": "Tomate",
            "quantity": 100,
            "unit": "g",
            "calories": 18,
            "protein": 0.9,
            "carbs": 3.9,
            "fats": 0.2
        },
        {
            "name": "Maíz",
            "quantity": 50,
            "unit": "g",
            "calories": 43,
            "protein": 1.6,
            "carbs": 9.5,
            "fats": 0.6
        },
        {
            "name": "Atún (natural, escurrido)",
            "quantity": 175,
            "unit": "g",
            "calories": 175,
            "protein": 40.3,
            "carbs": 0,
            "fats": 1.8
        },
        {
            "name": "Huevo",
            "quantity": "3",
            "unit": "unidad",
            "calories": 225,
            "protein": 18,
            "carbs": 1.8,
            "fats": 15
        },
        {
            "name": "Aceite de Oliva",
            "quantity": "10",
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Cocer las patatas, pelar y trocear. 2. Cocer los huevos. 3. Picar todos los ingredientes y mezclar. 4. Servir todo junto en un bol y aliñar con aceite de oliva virgen extra, sal y otras especies al gusto.",
    "originalBaseRecipeId": "cc_ensala_de_patata_atn_y_huevo",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p3_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx1: {
    "id": "p3_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx1",
    "name": "Patatas Gajo al Horno o Air Fryer",
    "calories": 240,
    "protein": 5.2,
    "carbs": 43.5,
    "fats": 5.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cd8b854a1885423545b10.png",
    "ingredients": [
        {
            "name": "Patatas",
            "quantity": "200",
            "unit": "g",
            "calories": 154.67,
            "protein": 4,
            "carbs": 34.13,
            "fats": 0.27
        },
        {
            "name": "cebolla",
            "quantity": "100",
            "unit": "g",
            "calories": 40,
            "protein": 1.2,
            "carbs": 9.4,
            "fats": 0.2
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Precalentar el horno a 200°C (390°F). 2. Lavar bien las patatas, cortarlas a trozos al gusto junto con la cebolla 3. Asar las patatas directamente sobre la rejilla del horno o en una bandeja durante 45-60 minutos (el tiempo dependerá del tamaño).  3. Servir todo junto en un bol y aliñar con aceite de oliva virgen extra, sal y otras especies al gusto.",
    "originalBaseRecipeId": "pp_patatas_gajo_al_horno_o_air_fryer",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p3_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx2: {
    "id": "p3_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx2",
    "name": "Solomillo/Filete de Ternera a la Plancha",
    "calories": 345,
    "protein": 50,
    "carbs": 0,
    "fats": 15,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afe6c0ed5060d3743015a.png",
    "ingredients": [
        {
            "name": "Solomillo de Ternera",
            "quantity": "200",
            "unit": "g",
            "calories": 300,
            "protein": 50,
            "carbs": 0,
            "fats": 10
        },
        {
            "name": "Aceite de Oliva",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Sacar el filete o solomillo de ternera del refrigerador unos 15-20 minutos antes de cocinar para que se temple. 2. Sazonar la carne con sal (preferiblemente en escamas después de cocinarla) y pimienta negra recién molida justo antes de ponerla en la plancha. 3. Calentar una plancha o sartén de fondo grueso a fuego alto hasta que esté bien caliente. Añadir una gota de aceite de oliva y extenderlo. 4. Cocinar el filete al gusto: Para poco hecho, 1-2 minutos por lado. Al punto, 2-3 minutos por lado. Bien hecho, 3-4 minutos o más por lado, dependiendo del grosor. Es importante no mover la carne mientras se sella la primera cara para obtener un buen dorado. 5. Dejar reposar la carne un par de minutos antes de servir para que los jugos se redistribuyan.",
    "originalBaseRecipeId": "ac_solomillofilete_de_ternera_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p3_extra_ext_sandwich_vegetal_de_jamn_york_Sandwich_Vegeta_mainOnly_idx3: {
    "id": "p3_extra_ext_sandwich_vegetal_de_jamn_york_Sandwich_Vegeta_mainOnly_idx3",
    "name": "Sandwich Vegetal de Jamón York",
    "calories": 165,
    "protein": 18.9,
    "carbs": 15.6,
    "fats": 3.2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cd5829b0c67f95e9a8e7e.png",
    "ingredients": [
        {
            "name": "Pan de Centeno",
            "quantity": 25,
            "unit": "g",
            "calories": 63,
            "protein": 2,
            "carbs": 12,
            "fats": 0.5
        },
        {
            "name": "Jamón York",
            "quantity": "80",
            "unit": "g",
            "calories": 88.53,
            "protein": 16,
            "carbs": 0.85,
            "fats": 2.45
        },
        {
            "name": "Tomate",
            "quantity": 50,
            "unit": "g",
            "calories": 9,
            "protein": 0.5,
            "carbs": 2,
            "fats": 0.1
        },
        {
            "name": "Lechuga",
            "quantity": 25,
            "unit": "g",
            "calories": 4,
            "protein": 0.4,
            "carbs": 0.7,
            "fats": 0.1
        }
    ],
    "preparation": "1. Tostar ligeramente las rebanadas de pan (opcional). 2. Lavar y picar la lechuga y el tomate. 3. Montar el sándwich alternando las capas de jamón york, tomate y lechuga entre las rebanadas de pan.",
    "originalBaseRecipeId": "ext_sandwich_vegetal_de_jamn_york",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4: {
    "id": "p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4",
    "name": "Fruta Densa (Plátano, Manzana)",
    "calories": 105,
    "protein": 1.3,
    "carbs": 27,
    "fats": 0.3,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afe4bb91eb325b93d4a82.png",
    "ingredients": [
        {
            "name": "Plátano",
            "quantity": 1,
            "unit": "unidad",
            "calories": 105,
            "protein": 1.3,
            "carbs": 27,
            "fats": 0.3
        }
    ],
    "preparation": "Lavar la fruta. Pelar si es necesario (ej. plátano). Consumir directamente.",
    "originalBaseRecipeId": "ext_fruta_densa_pltano_manzana",
    "isSideDish": false,
    "mealCategory": "extra"
}
};

const dailyMenus = [
  {
    name: "Entreno 1",
    targetMacros: {"calories":2000,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p0_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0'],
    comida: ['p0_comida_cc_patatas_cocidas_con_salteado_de_verduras_y_hamburguesa_a_la_plancha_Patatas_Cocidas_mainOnly_idx1'],
    cena: ['p0_cena_cc_poke_de_arroz_con_atn_queso_feta_tomate_y_aguacate_Poke_de_Arroz_c_mainOnly_idx2'],
    extra: ['p0_extra_ext_yogur_protico_sabores_natural_Yogur_Prot_ico__mainOnly_idx3', 'p0_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4'],
  },
  {
    name: "Entreno 2",
    targetMacros: {"calories":2000,"protein":150,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p1_desayuno_des_tostada_de_centeno_con_aguacate_y_atn_Tostada_de_Cent_mainOnly_idx0'],
    comida: ['p1_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx1'],
    cena: ['p1_cena_pp_pimientos_salteados_con_cebolla_y_tomate_cherry_Pimientos_Salte_mainOnly_idx2', 'p1_cena_ac_merluza_a_la_plancha_Merluza_a_la_Pl_side_idx3'],
    extra: ['p1_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx4', 'p1_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx5'],
  },
  {
    name: "Libre",
    targetMacros: {"calories":1800,"protein":160,"carbs":100},
    useCustomMacros: true,
    desayuno: ['p2_desayuno_des_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx0'],
    comida: ['p2_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx1', 'p2_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx2'],
    cena: ['p2_cena_cc_patata_con_brcoli_y_pollo_asado_Patata_con_br_c_mainOnly_idx3'],
    extra: ['p2_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx4'],
  },
  {
    name: "Ayuno",
    targetMacros: {"calories":1800,"protein":160,"carbs":100},
    useCustomMacros: true,
    comida: ['p3_comida_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx0'],
    cena: ['p3_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx1', 'p3_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx2'],
    extra: ['p3_extra_ext_sandwich_vegetal_de_jamn_york_Sandwich_Vegeta_mainOnly_idx3', 'p3_extra_ext_fruta_densa_pltano_manzana_Fruta_Densa__Pl_mainOnly_idx4'],
  }
];

// Detalles de los planes exportados:
// Plan: Entreno 1, Calorías Objetivo: 2000, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150
// Plan: Entreno 2, Calorías Objetivo: 2000, Proteínas Objetivo: 150, Carbohidratos Objetivo: 150
// Plan: Libre, Calorías Objetivo: 1800, Proteínas Objetivo: 160, Carbohidratos Objetivo: 100
// Plan: Ayuno, Calorías Objetivo: 1800, Proteínas Objetivo: 160, Carbohidratos Objetivo: 100
