// Archivo de datos NutriPlan AI. Generado para carga directa.
// La variable 'foodCategories' no se incluye aquí ya que se importa desde la app.

const foodDatabase = {
    p0_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx0: {
    "id": "p0_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx0",
    "name": "Poke de aguacate, mango, tomate, soja, cebolla, arroz y pollo",
    "calories": 760,
    "protein": 54.4,
    "carbs": 81,
    "fats": 27.1,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684efa4ef6c48db833a5b1af.png",
    "ingredients": [
        {
            "name": "Pechuga de Pollo (cruda)",
            "quantity": "200",
            "unit": "g",
            "calories": 220.57,
            "protein": 45.71,
            "carbs": 0,
            "fats": 4.57
        },
        {
            "name": "Arroz (crudo)",
            "quantity": "75",
            "unit": "g",
            "calories": 270,
            "protein": 6,
            "carbs": 60,
            "fats": 1.5
        },
        {
            "name": "Aguacate",
            "quantity": 75,
            "unit": "g",
            "calories": 120,
            "protein": 2,
            "carbs": 7,
            "fats": 11
        },
        {
            "name": "Mango",
            "quantity": 50,
            "unit": "g",
            "calories": 30,
            "protein": 0,
            "carbs": 8,
            "fats": 0
        },
        {
            "name": "Tomate",
            "quantity": "50",
            "unit": "g",
            "calories": 9.33,
            "protein": 0.67,
            "carbs": 2,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": "50",
            "unit": "g",
            "calories": 20,
            "protein": 0,
            "carbs": 4,
            "fats": 0
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": "10",
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "Cocinar el arroz y el pollo. Cortar el resto de los ingredientes y mezclarlo todo. Aderezar con aceite de oliva.",
    "originalBaseRecipeId": "cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p0_cena_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx1: {
    "id": "p0_cena_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx1",
    "name": "Ensala de Patata, Atún y Huevo",
    "calories": 510,
    "protein": 47.7,
    "carbs": 39,
    "fats": 16.7,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afc08b91eb31afe3d4796.png",
    "ingredients": [
        {
            "name": "Patata (cruda)",
            "quantity": "150",
            "unit": "g",
            "calories": 116,
            "protein": 4,
            "carbs": 26,
            "fats": 0
        },
        {
            "name": "Pimiento Verde",
            "quantity": "50",
            "unit": "g",
            "calories": 10,
            "protein": 0,
            "carbs": 2,
            "fats": 0
        },
        {
            "name": "Pimiento Rojo",
            "quantity": "50",
            "unit": "g",
            "calories": 16,
            "protein": 0,
            "carbs": 4,
            "fats": 0
        },
        {
            "name": "Cebolla",
            "quantity": "50",
            "unit": "g",
            "calories": 20,
            "protein": 0,
            "carbs": 4,
            "fats": 0
        },
        {
            "name": "Tomate",
            "quantity": 50,
            "unit": "g",
            "calories": 9,
            "protein": 0,
            "carbs": 2,
            "fats": 0
        },
        {
            "name": "Atún (en conserva al natural, escurrido)",
            "quantity": "125",
            "unit": "g",
            "calories": 137.5,
            "protein": 31.67,
            "carbs": 0,
            "fats": 1.67
        },
        {
            "name": "Huevo",
            "quantity": 2,
            "unit": "unidad",
            "calories": 156,
            "protein": 12,
            "carbs": 1,
            "fats": 10
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "Cocinar las patatas y los huevos. Trocear las verduras, el atún y los huevos cocidos. Mezclar todo y aderezar con aceite de oliva.",
    "originalBaseRecipeId": "cc_ensala_de_patata_atn_y_huevo",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p0_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx2: {
    "id": "p0_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx2",
    "name": "Fruta Cítrica (Mandarina, Kiwi, Naranja...)",
    "calories": 112,
    "protein": 2,
    "carbs": 28,
    "fats": 0,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afd3d0ed506305442ffe4.png",
    "ingredients": [
        {
            "name": "Kiwi",
            "quantity": 1,
            "unit": "unidad",
            "calories": 42,
            "protein": 1,
            "carbs": 10,
            "fats": 0
        },
        {
            "name": "Naranja",
            "quantity": 1,
            "unit": "unidad",
            "calories": 70,
            "protein": 1,
            "carbs": 18,
            "fats": 0
        }
    ],
    "preparation": "Pelar y consumir la fruta.",
    "originalBaseRecipeId": "ext_fruta_ctrica_mandarina_kiwi_naranja",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p0_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx3: {
    "id": "p0_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx3",
    "name": "Yogurt Protéico con Frutos Rojos y Secos",
    "calories": 311,
    "protein": 18,
    "carbs": 25,
    "fats": 17,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afea2b91eb35e413d4ab6.png",
    "ingredients": [
        {
            "name": "Yogur Protéico",
            "quantity": 125,
            "unit": "g",
            "calories": 90,
            "protein": 11,
            "carbs": 7,
            "fats": 1
        },
        {
            "name": "Frutos Rojos (congelados)",
            "quantity": 100,
            "unit": "g",
            "calories": 50,
            "protein": 1,
            "carbs": 12,
            "fats": 1
        },
        {
            "name": "Almendras",
            "quantity": "30",
            "unit": "g",
            "calories": 171,
            "protein": 6,
            "carbs": 6,
            "fats": 15
        }
    ],
    "preparation": "Mezclar el yogur protéico con los frutos rojos y los frutos secos.",
    "originalBaseRecipeId": "ext_yogurt_protico_con_frutos_rojos_y_secos",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p1_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx0: {
    "id": "p1_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx0",
    "name": "Ensalada de Lentejas",
    "calories": 621,
    "protein": 41,
    "carbs": 73.7,
    "fats": 23,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682aff5a365e949c0a73d706.png",
    "ingredients": [
        {
            "name": "Lentejas cocidas",
            "quantity": 250,
            "unit": "g",
            "calories": 290,
            "protein": 22.5,
            "carbs": 50,
            "fats": 1
        },
        {
            "name": "Tomate",
            "quantity": 100,
            "unit": "g",
            "calories": 18,
            "protein": 0.9,
            "carbs": 3.9,
            "fats": 0.2
        },
        {
            "name": "Cebolla",
            "quantity": "100",
            "unit": "g",
            "calories": 40,
            "protein": 1.1,
            "carbs": 9.3,
            "fats": 0.1
        },
        {
            "name": "Queso de Burgos 0%",
            "quantity": "125",
            "unit": "g",
            "calories": 62.5,
            "protein": 15,
            "carbs": 3.75,
            "fats": 0.63
        },
        {
            "name": "Aguacate",
            "quantity": 75,
            "unit": "g",
            "calories": 120,
            "protein": 1.5,
            "carbs": 6.75,
            "fats": 11.03
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Si usas lentejas de bote, enjuagarlas bien bajo el grifo y escurrirlas. Si las cocinas, déjalas enfriar. 2. Picar el tomate y la cebolla (preferiblemente morada) en dados pequeños. Cortar el queso de Burgos 0% y el aguacate en dados. 3. En un bol grande, mezclar con cuidado las lentejas, el tomate, la cebolla, el queso de Burgos y el aguacate. 4. Aliñar con aceite de oliva virgen extra, vinagre (de manzana o Jerez) y sal al gusto. Opcional: añadir cilantro fresco picado o comino molido para un toque diferente.",
    "originalBaseRecipeId": "pp_ensalada_de_lentejas",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p1_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx1: {
    "id": "p1_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx1",
    "name": "Pechuga de Pavo a la Plancha",
    "calories": 206,
    "protein": 35,
    "carbs": 0,
    "fats": 7,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f003ef6c48dfc43a5bdb3.png",
    "ingredients": [
        {
            "name": "Pechuga de Pavo",
            "quantity": 150,
            "unit": "g",
            "calories": 160.5,
            "protein": 34.5,
            "carbs": 0,
            "fats": 2.25
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Si los filetes de pechuga de pavo son muy gruesos, se pueden abrir tipo libro o golpear ligeramente para que tengan un grosor más uniforme y se cocinen mejor. 2. Sazonar los filetes de pavo con sal, pimienta y tus especias favoritas (ej. orégano, pimentón dulce, curry en polvo, hierbas provenzales). 3. Calentar una plancha o sartén antiadherente a fuego medio-alto con una gota de aceite de oliva. 4. Cocinar la pechuga de pavo durante unos 3-4 minutos por cada lado, o hasta que esté dorada por fuera y completamente cocida por dentro (sin partes rosadas). El tiempo exacto dependerá del grosor.",
    "originalBaseRecipeId": "ac_pechuga_de_pavo_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p1_cena_pp_crema_de_verduras_variadas_Crema_de_Verdur_mainOnly_idx2: {
    "id": "p1_cena_pp_crema_de_verduras_variadas_Crema_de_Verdur_mainOnly_idx2",
    "name": "Crema de Verduras Variadas",
    "calories": 126,
    "protein": 3,
    "carbs": 20,
    "fats": 5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cd68425d68cc771ebd10a.png",
    "ingredients": [
        {
            "name": "Calabaza",
            "quantity": 200,
            "unit": "g",
            "calories": 52,
            "protein": 2,
            "carbs": 13,
            "fats": 0.2
        },
        {
            "name": "Tomate",
            "quantity": 50,
            "unit": "g",
            "calories": 9,
            "protein": 0.45,
            "carbs": 1.95,
            "fats": 0.1
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.55,
            "carbs": 4.65,
            "fats": 0.05
        },
        {
            "name": "Agua",
            "quantity": 200,
            "unit": "ml",
            "calories": 0,
            "protein": 0,
            "carbs": 0,
            "fats": 0
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Puedes comprar una crema preparada o bien cocinar unas cuantas verduras en una olla.",
    "originalBaseRecipeId": "pp_crema_de_verduras_variadas",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3: {
    "id": "p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3",
    "name": "Solomillo/Filete de Ternera a la Plancha",
    "calories": 240,
    "protein": 32,
    "carbs": 0,
    "fats": 13,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afe6c0ed5060d3743015a.png",
    "ingredients": [
        {
            "name": "Solomillo de Ternera",
            "quantity": 150,
            "unit": "g",
            "calories": 195,
            "protein": 31.5,
            "carbs": 0,
            "fats": 7.5
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Sacar el filete o solomillo de ternera del refrigerador unos 15-20 minutos antes de cocinar para que se temple. 2. Sazonar la carne con sal (preferiblemente en escamas después de cocinarla) y pimienta negra recién molida justo antes de ponerla en la plancha. 3. Calentar una plancha o sartén de fondo grueso a fuego alto hasta que esté bien caliente. Añadir una gota de aceite de oliva y extenderlo. 4. Cocinar el filete al gusto: Para poco hecho, 1-2 minutos por lado. Al punto, 2-3 minutos por lado. Bien hecho, 3-4 minutos o más por lado, dependiendo del grosor. Es importante no mover la carne mientras se sella la primera cara para obtener un buen dorado. 5. Dejar reposar la carne un par de minutos antes de servir para que los jugos se redistribuyan.",
    "originalBaseRecipeId": "ac_solomillofilete_de_ternera_a_la_plancha",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p1_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4: {
    "id": "p1_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4",
    "name": "Queso Fresco Batido con Frutos Rojos y Miel",
    "calories": 261,
    "protein": 14,
    "carbs": 54,
    "fats": 2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/6879302d02da47667d446b43.png",
    "ingredients": [
        {
            "name": "Queso Fresco Batido",
            "quantity": 125,
            "unit": "g",
            "calories": 70,
            "protein": 12,
            "carbs": 5,
            "fats": 0.5
        },
        {
            "name": "Frutos Rojos",
            "quantity": 200,
            "unit": "g",
            "calories": 100,
            "protein": 2,
            "carbs": 24,
            "fats": 1
        },
        {
            "name": "Miel Cruda",
            "quantity": 30,
            "unit": "g",
            "calories": 91.2,
            "protein": 0.09,
            "carbs": 24.6,
            "fats": 0
        }
    ],
    "preparation": "1. En un bol, mezclar el queso fresco batido con los frutos rojos. 2. Añadir la miel al gusto y mezclar suavemente.",
    "originalBaseRecipeId": "ext_queso_fresco_batido_con_frutos_rojos_y_miel",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p1_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx5: {
    "id": "p1_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx5",
    "name": "Batido Whey de Proteínas",
    "calories": 114,
    "protein": 24,
    "carbs": 2,
    "fats": 2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f021af6c48d41b5a5c003.png",
    "ingredients": [
        {
            "name": "Proteína en Polvo",
            "quantity": 30,
            "unit": "g",
            "calories": 114,
            "protein": 24,
            "carbs": 1.5,
            "fats": 1.5
        },
        {
            "name": "Agua",
            "quantity": 200,
            "unit": "ml",
            "calories": 0,
            "protein": 0,
            "carbs": 0,
            "fats": 0
        }
    ],
    "preparation": "1. Mezclar la proteína en polvo con agua en un shaker o batidora hasta obtener una consistencia homogénea.",
    "originalBaseRecipeId": "ext_batido_whey_de_protenas",
    "isSideDish": false,
    "mealCategory": "extra"
},
    p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0: {
    "id": "p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0",
    "name": "Revuelto de Huevos con Jamón",
    "calories": 395,
    "protein": 33,
    "carbs": 2,
    "fats": 27.5,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682ad5e1e819fcb589a8a30e.png",
    "ingredients": [
        {
            "name": "Huevos",
            "quantity": 3,
            "unit": "unidad",
            "calories": 225,
            "protein": 18,
            "carbs": 2,
            "fats": 15
        },
        {
            "name": "Jamón Serrano",
            "quantity": 50,
            "unit": "g",
            "calories": 80,
            "protein": 15,
            "carbs": 0,
            "fats": 2.5
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 10,
            "unit": "g",
            "calories": 90,
            "protein": 0,
            "carbs": 0,
            "fats": 10
        }
    ],
    "preparation": "1. Calentar una sartén antiadherente a fuego medio con un poco de aceite de oliva. 2. Batir los huevos en un bol y añadir una pizca de sal y pimienta. 3. Añadir el jamón cortado en dados a la sartén y cocinar por un minuto. 4. Verter los huevos batidos sobre el jamón. 5. Cocinar, removiendo suavemente con una espátula hasta que los huevos estén cuajados pero aún cremosos. 6. Servir inmediatamente.",
    "originalBaseRecipeId": "des_revuelto_de_huevos_con_jamn",
    "isSideDish": false,
    "mealCategory": "desayuno"
},
    p2_comida_cc_ensalada_con_pollo_y_queso_de_burgos_Ensalada_con_Po_mainOnly_idx1: {
    "id": "p2_comida_cc_ensalada_con_pollo_y_queso_de_burgos_Ensalada_con_Po_mainOnly_idx1",
    "name": "Ensalada con Pollo y Queso de Burgos",
    "calories": 437,
    "protein": 50.5,
    "carbs": 17,
    "fats": 18.7,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afdceb91eb360843d4a0b.png",
    "ingredients": [
        {
            "name": "Pechuga de Pollo",
            "quantity": 150,
            "unit": "g",
            "calories": 165,
            "protein": 34.5,
            "carbs": 0,
            "fats": 3
        },
        {
            "name": "Queso de Burgos 0%",
            "quantity": 100,
            "unit": "g",
            "calories": 70,
            "protein": 12,
            "carbs": 3,
            "fats": 0
        },
        {
            "name": "Canónigos",
            "quantity": 100,
            "unit": "g",
            "calories": 20,
            "protein": 2,
            "carbs": 3,
            "fats": 0.3
        },
        {
            "name": "Tomate",
            "quantity": 150,
            "unit": "g",
            "calories": 27,
            "protein": 1.4,
            "carbs": 6,
            "fats": 0.3
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.6,
            "carbs": 5,
            "fats": 0.1
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 15,
            "unit": "g",
            "calories": 135,
            "protein": 0,
            "carbs": 0,
            "fats": 15
        }
    ],
    "preparation": "1. Lavar y escurrir bien los canónigos, el tomate y la cebolla. 2. Cortar el tomate y la cebolla en rodajas finas o dados, según preferencia. 3. Cortar la pechuga de pollo en tiras o cubos y cocinar a la plancha hasta que esté dorada y cocida. 4. Cortar el queso de Burgos en dados. 5. En un bol grande, combinar los canónigos, tomate, cebolla, pollo y queso de Burgos. 6. Aliñar con aceite de oliva virgen extra, sal y especias al gusto antes de servir.",
    "originalBaseRecipeId": "cc_ensalada_con_pollo_y_queso_de_burgos",
    "isSideDish": false,
    "mealCategory": "comida_cena_completa"
},
    p2_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx2: {
    "id": "p2_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx2",
    "name": "Patatas Gajo al Horno o Air Fryer",
    "calories": 386,
    "protein": 6.6,
    "carbs": 56,
    "fats": 15.4,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/687cd8b854a1885423545b10.png",
    "ingredients": [
        {
            "name": "Patatas",
            "quantity": 300,
            "unit": "g",
            "calories": 231,
            "protein": 6,
            "carbs": 51,
            "fats": 0.3
        },
        {
            "name": "Cebolla",
            "quantity": 50,
            "unit": "g",
            "calories": 20,
            "protein": 0.6,
            "carbs": 5,
            "fats": 0.1
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 15,
            "unit": "g",
            "calories": 135,
            "protein": 0,
            "carbs": 0,
            "fats": 15
        }
    ],
    "preparation": "1. Precalentar el horno a 200°C (390°F). 2. Lavar bien las patatas, cortarlas a trozos al gusto junto con la cebolla 3. Asar las patatas directamente sobre la rejilla del horno o en una bandeja durante 45-60 minutos (el tiempo dependerá del tamaño).  3. Servir todo junto en un bol y aliñar con aceite de oliva virgen extra, sal y otras especies al gusto.",
    "originalBaseRecipeId": "pp_patatas_gajo_al_horno_o_air_fryer",
    "isSideDish": false,
    "mealCategory": "plato_principal"
},
    p2_cena_ac_hamburguesa_de_pavopollo_Hamburguesa_de__side_idx3: {
    "id": "p2_cena_ac_hamburguesa_de_pavopollo_Hamburguesa_de__side_idx3",
    "name": "Hamburguesa de Pavo-Pollo",
    "calories": 233,
    "protein": 25,
    "carbs": 0,
    "fats": 15,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/682afcbc9b8ed3b6397cbb33.png",
    "ingredients": [
        {
            "name": "Hamburguesa de Pavo-Pollo",
            "quantity": 125,
            "unit": "g",
            "calories": 188,
            "protein": 25,
            "carbs": 0,
            "fats": 10
        },
        {
            "name": "Aceite de Oliva Virgen Extra",
            "quantity": 5,
            "unit": "g",
            "calories": 45,
            "protein": 0,
            "carbs": 0,
            "fats": 5
        }
    ],
    "preparation": "1. Si la hamburguesa es casera a partir de carne picada, sazonar la carne con sal, pimienta, ajo en polvo y perejil picado (opcional) y formar la hamburguesa. Si es comprada, ya está lista. 2. Calentar una plancha o sartén antiadherente a fuego medio-alto con una gota de aceite de oliva (opcional, si la sartén no es muy antiadherente). 3. Cocinar la hamburguesa durante 4-6 minutos por cada lado, o hasta que esté bien cocida por dentro (temperatura interna de 74°C o 165°F) y dorada por fuera. Evitar aplastarla para que no pierda jugosidad.",
    "originalBaseRecipeId": "ac_hamburguesa_de_pavopollo",
    "isSideDish": true,
    "mealCategory": "acompanamiento"
},
    p2_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx4: {
    "id": "p2_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx4",
    "name": "Batido Whey de Proteínas",
    "calories": 111,
    "protein": 24,
    "carbs": 2,
    "fats": 2,
    "image": "https://storage.googleapis.com/msgsndr/dikOTQ4DE3OClw85d5oB/media/684f021af6c48d41b5a5c003.png",
    "ingredients": [
        {
            "name": "Proteína en Polvo (Whey)",
            "quantity": 30,
            "unit": "g",
            "calories": 111,
            "protein": 24,
            "carbs": 2,
            "fats": 2
        },
        {
            "name": "Agua",
            "quantity": 200,
            "unit": "ml",
            "calories": 0,
            "protein": 0,
            "carbs": 0,
            "fats": 0
        }
    ],
    "preparation": "1. En un mezclador o batidora, añadir el agua. 2. Incorporar la proteína en polvo. 3. Agitar o batir enérgicamente hasta que no queden grumos y la mezcla sea homogénea. 4. Consumir inmediatamente.",
    "originalBaseRecipeId": "ext_batido_whey_de_protenas",
    "isSideDish": false,
    "mealCategory": "extra"
}
};

const dailyMenus = [
  {
    name: "Ayuno 1",
    targetMacros: {"calories":1700,"protein":140,"carbs":150},
    useCustomMacros: false,
    comida: ['p0_comida_cc_poke_de_aguacate_mango_tomate_soja_cebolla_arroz_y_pollo_Poke_de_aguacat_mainOnly_idx0'],
    cena: ['p0_cena_cc_ensala_de_patata_atn_y_huevo_Ensala_de_Patat_mainOnly_idx1'],
    extra: ['p0_extra_ext_fruta_ctrica_mandarina_kiwi_naranja_Fruta_C_trica___mainOnly_idx2', 'p0_extra_ext_yogurt_protico_con_frutos_rojos_y_secos_Yogurt_Prot_ico_mainOnly_idx3'],
  },
  {
    name: "Ayuno 2",
    targetMacros: {"calories":1700,"protein":140,"carbs":150},
    useCustomMacros: false,
    comida: ['p1_comida_pp_ensalada_de_lentejas_Ensalada_de_Len_mainOnly_idx0', 'p1_comida_ac_pechuga_de_pavo_a_la_plancha_Pechuga_de_Pavo_side_idx1'],
    cena: ['p1_cena_pp_crema_de_verduras_variadas_Crema_de_Verdur_mainOnly_idx2', 'p1_cena_ac_solomillofilete_de_ternera_a_la_plancha_Solomillo_Filet_side_idx3'],
    extra: ['p1_extra_ext_queso_fresco_batido_con_frutos_rojos_y_miel_Queso_Fresco_Ba_mainOnly_idx4', 'p1_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx5'],
  },
  {
    name: "Completo 1",
    targetMacros: {"calories":1700,"protein":140,"carbs":150},
    useCustomMacros: false,
    desayuno: ['p2_desayuno_des_revuelto_de_huevos_con_jamn_Revuelto_de_Hue_mainOnly_idx0'],
    comida: ['p2_comida_cc_ensalada_con_pollo_y_queso_de_burgos_Ensalada_con_Po_mainOnly_idx1'],
    cena: ['p2_cena_pp_patatas_gajo_al_horno_o_air_fryer_Patatas_Gajo_al_mainOnly_idx2', 'p2_cena_ac_hamburguesa_de_pavopollo_Hamburguesa_de__side_idx3'],
    extra: ['p2_extra_ext_batido_whey_de_protenas_Batido_Whey_de__mainOnly_idx4'],
  }
];

// Detalles de los planes exportados:
// Plan: Ayuno 1, Calorías Objetivo: 1700, Proteínas Objetivo: 140, Carbohidratos Objetivo: 150
// Plan: Ayuno 2, Calorías Objetivo: 1700, Proteínas Objetivo: 140, Carbohidratos Objetivo: 150
// Plan: Completo 1, Calorías Objetivo: 1700, Proteínas Objetivo: 140, Carbohidratos Objetivo: 150
